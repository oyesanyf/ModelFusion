"""
fix_slash_commands.py — Comprehensive slash command and @agent detection patch for extension.js
================================================================================================
Replaces slash command extraction logic with complete multi-strategy matcher:
  - cleanUserText() to strip XML context tags
  - knownCommands Set with all registered commands + typo aliases
  - extractKnownCmd() matching @agent /cmd, @agent cmd, /cmd, and alias normalization
  - Compaction fast handler
"""
...
NEW_BLOCK = '\n'.join([
    ...
    r'let clnUT=function(S){if(!S)return"";let X=S;X=X.replace(/\[Context: Selected Explorer Item\(s\):[\s\S]*?\]/gi," ");X=X.replace(/<attachments[\s\S]*?<\/attachments>/gi," ");X=X.replace(/<attachment[\s\S]*?<\/attachment>/gi," ");X=X.replace(/<attachments[\s\S]*?>/gi," ");X=X.replace(/<attachment[\s\S]*?>/gi," ");X=X.replace(/<environment_info[\s\S]*?<\/environment_info>/gi," ");X=X.replace(/<workspace_info[\s\S]*?<\/workspace_info>/gi," ");X=X.replace(/<context[\s\S]*?<\/context>/gi," ");X=X.replace(/<editorContext[\s\S]*?<\/editorContext>/gi," ");X=X.replace(/<reminderInstructions[\s\S]*?<\/reminderInstructions>/gi," ");X=X.replace(/<customizationsUpdate[\s\S]*?<\/customizationsUpdate>/gi," ");X=X.replace(/<conversation-summary[\s\S]*?<\/conversation-summary>/gi," ");return X.trim();};',
    r'let extKC=function(S){if(!S)return"";let am=S.match(/@agent\s+\/?([a-zA-Z][\w-]*)\s*([\s\S]*)/i)||S.match(/@commands\s+\/?([a-zA-Z][\w-]*)\s*([\s\S]*)/i);if(am){let raw=am[1].toLowerCase();let cn=normCmd(raw);if(KC.has(raw)||KC.has(cn))return"/"+cn+" "+(am[2]||"").trim();}let sm=[...S.matchAll(/\/([a-zA-Z][\w-]*)\b\s*([\s\S]*)/gi)];for(let m of sm){let raw=m[1].toLowerCase();let cn=normCmd(raw);if(KC.has(raw)||KC.has(cn))return"/"+cn+" "+(m[2]||"").trim();}let cl=clnUT(S);if(cl){let ws=cl.split(/\s+/);for(let w of ws){let raw=w.toLowerCase().replace(/[^a-z0-9_-]/g,"");let cn=normCmd(raw);if(KC.has(raw)||KC.has(cn))return"/"+cn;}}return"";};',
