...
NEW_BLOCK = '\n'.join([
    ...
    r'let extKC=function(S){if(!S)return"";let cl=clnUT(S);if(!cl)return"";let am=cl.match(/^@(?:agent|commands?|comments?|tasks?|modelfusion|hugos|code)\s+\/?([a-zA-Z][\w-]*)\s*([\s\S]*)/i);if(am){let raw=am[1].toLowerCase();let cn=normCmd(raw);if(KC.has(raw)||KC.has(cn))return"/"+cn+(am[2]?" "+am[2].trim():"");}let dcm=cl.match(/^@(?:comments?)\b\s*([\s\S]*)/i);if(dcm)return"/comment"+(dcm[1]?" "+dcm[1].trim():"");let dam=cl.match(/^@(?:agent|commands?|tasks?|modelfusion|hugos)\b\s*([\s\S]*)/i);if(dam){let rest=dam[1].trim();if(!rest)return"/stats";let fw=rest.split(/\s+/)[0].replace(/^[\/@]/,"").toLowerCase();let cn=normCmd(fw);if(KC.has(fw)||KC.has(cn)){let aw=rest.slice(rest.indexOf(fw)+fw.length).trim();return"/"+cn+(aw?" "+aw:"");}}let sm=cl.match(/^\/([a-zA-Z][\w-]*)\s*([\s\S]*)/i);if(sm){let raw=sm[1].toLowerCase();let cn=normCmd(raw);if(KC.has(raw)||KC.has(cn))return"/"+cn+(sm[2]?" "+sm[2].trim():"");}let ws=cl.split(/\s+/);if(ws.length>0){let raw=ws[0].toLowerCase().replace(/^[\/@]/,"").replace(/[^a-z0-9_-]/g,"");let cn=normCmd(raw);if(KC.has(raw)||KC.has(cn)){let rest=ws.slice(1).join(" ").trim();return"/"+cn+(rest?" "+rest:"");}}return"";};',
