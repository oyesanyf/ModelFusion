...
replacement_str = 'setTimeout(async()=>{try{Gi.mkdirSync(no.dirname(s),{recursive:!0});let g=no.join(r,"db","hf_models.db");if(Gi.existsSync(g)){let f=Gi.existsSync(s),h=f?Gi.statSync(s).size:0;if(!f||h<1e5){let y=`Copying pre-populated database from ${g} to ${s}...`;this._logService.info(`ModelFusionProvider: ${y}`),this._outputChannel.appendLine(`[DB] ${y}`),Gi.copyFileSync(g,s),this._logService.info("ModelFusionProvider: Database successfully pre-populated!"),this._outputChannel.appendLine("[DB] Database successfully pre-populated!")}}}catch(g){this._outputChannel.appendLine(`[DB ERROR] ${g.message}`)}let c=o.get("ovModelDir","")||no.join(gg.homedir(),".hugos-ide","ov_models");try{let g=no.join(r,"ov_models");if(Gi.existsSync(g)){let f=Gi.readdirSync(g).filter(h=>Gi.statSync(no.join(g,h)).isDirectory());for(let h of f){let y=no.join(c,h);if(!Gi.existsSync(y)){this._outputChannel.appendLine(`[OV] First launch: copying bundled model ${h} to ${y}...`),Gi.mkdirSync(y,{recursive:!0});let v=no.join(g,h);for(let _ of Gi.readdirSync(v)){let w=no.join(v,_);Gi.statSync(w).isFile()&&Gi.copyFileSync(w,no.join(y,_))}this._outputChannel.appendLine(`[OV] Copied starter model: ${h}`)}}}}catch(g){this._outputChannel.appendLine(`[OV] Warning: Could not copy bundled models: ${g.message}`)}},10);let l=o.get("getvino",!1);'
