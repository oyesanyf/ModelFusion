...
BRANCH = "feat/acdso-cli-integration-build-150"
...
def main():
    ...
    commit_msg = "feat(cli): adapt ACDSO risk-aware AutoML CLI, add --acdso and @agent acdso, package signed MSI 150"
    ...
    pr_data = {
        "title": commit_msg,
        "body": "## Summary\n- Integrate ACDSO (Adaptive Contextual Data Science Optimization) Risk-Aware AutoML into ModelFusion CLI as `--acdso` and `@agent acdso` / `/acdso`\n- Support `--target`, `--predict`, `--best-score`, `--timeseries`, `--datetime-col`, `--horizon`, `--decision`, and `--treatment` flags\n- Update task processor with complete data science prompt enforcing GroupKFold leakage prevention, feature engineering, and multi-model benchmarking\n- Register `/acdso` in Copilot extension manifest and synchronize across all 5 distribution targets\n- Document all 170 production flags in `docs/CLI_REFERENCE.md` and `README.md`\n- Compile release CLI and maintain 100% 4-way cryptographic binary parity\n- Pass all 49 CLI unit tests and all 59 ReST-RL tests\n- Build and package Authenticode-signed HugOS MSI Build 150 with DigiCert timestamping",
        "head": BRANCH,
        "base": "main"
    }
    headers = {
