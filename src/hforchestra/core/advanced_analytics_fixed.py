#!/usr/bin/env python3
"""
Advanced SQLite Model Analytics and Query Patterns (Fixed Version)
================================================================

This module provides sophisticated querying, analytics, and optimization
patterns for your stored Hugging Face model data using only available columns.
"""
...
class AdvancedModelAnalytics:
    ...
    def setup_advanced_features(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Create advanced indexes (using actual column names)
                indexes = [
    ...
    def dynamic_model_ranking(self, 
        ...
        try:
            ...
            SELECT model_id, pipeline_tag, downloads, likes, license, size_mb, 
                   last_modified, tags, decision_score as score,
                   ...
                   CASE 
                       WHEN ? IS NOT NULL THEN 
                           ...
                           (? * CASE WHEN COALESCE(size_mb, 1000) < 500 THEN 1 ELSE 0 END) +
    ...
    def model_trend_analysis(self, days_back: int = 30) -> pd.DataFrame:
        ...
        try:
            ...
            WITH trend_data AS (
                SELECT 
                    pipeline_tag,
                    DATE(last_modified) as mod_date,
                    COUNT(*) as models_updated,
                    AVG(downloads) as avg_downloads,
                    AVG(likes) as avg_likes,
                    AVG(decision_score) as avg_score
                FROM models 
                WHERE last_modified >= date('now', '-{} days')
                GROUP BY pipeline_tag, DATE(last_modified)
            )
            SELECT 
                pipeline_tag,
    ...
    def similarity_clustering(self, model_id: str, top_k: int = 5) -> pd.DataFrame:
        """
        Find similar models using feature-based similarity (SQLite compatible)
        """
        try:
            query = """
            ...
            similarity_scores AS (
                SELECT 
                    ...
                    -- Calculate similarity score using SQLite-compatible functions
                    (1.0 - ABS(
    ...
    def create_model_recommendation_system(self, user_preferences: Dict) -> pd.DataFrame:
        ...
        try:
            ...
            if not tasks:  # If no tasks specified, don't add task filtering
                base_query = """
                SELECT 
                    model_id, pipeline_tag, downloads, likes, license, size_mb, decision_score as score,
                    -- Custom recommendation score
                    (decision_score * ?) + 
                    (downloads * 1.0 / NULLIF((SELECT MAX(downloads) FROM models), 0) * ?) as rec_score
                FROM models
                WHERE 1=1
                """
                params = [
