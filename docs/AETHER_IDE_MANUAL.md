# Aether IDE & ModelFusion Integration Manual

Aether IDE categorizes task queries by modality (text, security, legal, domain, image, and audio) and features native local multimodal processing directly through its Python execution pipeline, completely bypassing the need for any external container manager or Ollama installation.
...
Aether features a hybrid task detector in `detector.rs` combining exact regex keyword patterns and a Term-Frequency Vector Space Model (VSM).
